...
/* 可以使用 - [模糊查询]提取现实内容的element*/
function getElementsByInnerText2(abc) {
    var list = document.getElementsByTagName('*');
    var arr = [];
    for (var i = 0; i < list.length; i++) {
        if (list[i].childNodes[0] != null) {
            if (list[i].childNodes[0].nodeValue != null) {
                if (list[i].childNodes[0].nodeValue.indexOf(abc) != -1) {
                    //return list[i];
                    arr.push(list[i]);
                }
            }
...
function setSelectRangeText(ele) {
    //删除之前所有选取
